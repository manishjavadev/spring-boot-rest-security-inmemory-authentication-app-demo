...
public class AccountController {
	...
	@RequestMapping(value = "/findaccountbyadmin/{accountNumber}", method = RequestMethod.GET, produces = {
			MediaType.APPLICATION_XML_VALUE, MediaType.APPLICATION_JSON_VALUE })
	...
	@ApiOperation(value = "Find Account Info")
	@RequestMapping(value = "/findaccountbyuser/{accountNumber}", method = RequestMethod.GET, produces = {
			MediaType.APPLICATION_XML_VALUE, MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<Account> findAccountByUser(@PathVariable("accountNumber") Long accountNumber) {
		Account account = accountService.findAccount(new Long(accountNumber));
		return new ResponseEntity<Account>(account, HttpStatus.OK);
	}
